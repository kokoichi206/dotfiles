...
- **正しく出せるまで粘る**: クリーンなツール呼び出しが通るまで、正規の function-call チャネルで出し直し続ける。ただし下記「脱出条件」に達したら粘るのをやめてエスカレーションする。
...
同一セッション内で復旧試行（クリーンな出し直し）が **5 回連続で失敗** したら、
文脈汚染がセッション内では回復不能と判断して粘るのをやめる：

1. 復旧試行を停止する。それ以上の出し直し・ツール連打をしない。
2. ユーザーに **セッションの再作成（`/clear` または新セッション）** を明示的に提案して停止する。
   提案には「何回試行してどう失敗したか」を 1〜2 行で添える。
3. 停止前に可能なら `notes.md` に未復旧のまま脱出した旨を記録する（記録自体が失敗するならスキップしてよい）。

「連続」のカウントは、クリーンなツール呼び出しが 1 回でも通ればリセットする。
Stop hook の block が繰り返されている場合も、この脱出条件を優先してユーザーへの提案文を最終メッセージとして出す。
