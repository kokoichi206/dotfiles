---
name: audit-sessions
description: 過去の Claude Code セッションログから人間発話を抽出し、スキル化候補・設定/ルール化候補・lint ルール化候補・プロンプト改善の 4 カテゴリで並列分析する監査手順。期間を指定して定期実行できる。トリガー - セッションログ監査, 過去のログを見て改善点, スキル化した方がいいもの探して, lint に落とせるもの探して, ハーネス監査, プロンプト傾向分析, 直近1週間のログ監査, 定期監査
---
...
強制力は「散文ルール → スキル → hook → linter」の順で上がる。
AST で静的に判定できる規約は lint ルール化候補へ回し、散文（CLAUDE.md）や口頭指示に留めない。
