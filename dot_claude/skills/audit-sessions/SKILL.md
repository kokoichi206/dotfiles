...
- `apply [<レポートパス or issue 番号>]` → **ステージ2**（§6）。
  監査 issue でチェックされた候補だけをトリアージして PR / issue にする。
  引数省略時は dotfiles の最新のオープンな「セッション監査」issue を対象にする。
...
- **ステージ1 は PR を作らない。** 定期実行の通知・承認入口として dotfiles に
  「セッション監査 <END>」issue を 1 件立てる（既存のオープンな監査 issue があれば追記更新）。
- 監査 issue の本文は **候補 1 件 = タスクリスト 1 行**で書く。
  このチェックボックスがステージ2 の承認ゲートになる（チェックされた行だけが実装対象）:
...
- PR / issue 化はステージ2（§6）で、チェックによる承認後に行う。

## 6. apply モード（ステージ2・チェック承認制）

`/audit-sessions apply [<レポートパス or issue 番号>]` で呼ばれたときだけ実行する。
