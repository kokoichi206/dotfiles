...
- レポートは日本語で `~/ghq/github.com/kokoichi206/tasks/reports/audit-sessions-<END>.md` に書き出す。
- 定期実行の通知・承認入口として `kokoichi206/tasks` に「セッション監査 <END>」issue を 1 件立てる
  （既存のオープンな監査 issue があれば追記更新）。
  issue 本文にレポートを貼らず、レポートのパスを参照させる。
...
- チェック済み記法（`- ` + `[x]`）を issue 本文の散文中にリテラルで書かない。
  apply-approved の定期実行 precheck は本文全体を grep するため、説明文が承認 1 件として誤検知される。
- 既に着地した項目はチェックボックスにしない（箇条書きで残す）。
  チェック済みの行は「これから実装する」を意味し、残すと再実装される。
