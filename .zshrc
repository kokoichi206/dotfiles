...
# Claude Code custom skill management
DOTFILES_DIR="$HOME/ghq/github.com/kokoichi206/dotfiles"

# Try new Claude Code skill (create in ~/.claude/skills/ for experimentation)
claude-skill-try() {
    local skill_name="$1"
...
# Promote skill to dotfiles (move from ~/.claude/skills/ to dotfiles and symlink)
claude-skill-promote() {
    ...
    # Check if skill exists in temporary location
    ...
    # Check if skill is already a symlink (already promoted)
    ...
    # Check if skill already exists in dotfiles
...
# List Claude Code custom skills
claude-skill-list() {
...
# Try new Codex CLI skill (create in ~/.codex/skills/ for experimentation)
codex-skill-try() {
    local skill_name="$1"
    if [ -z "$skill_name" ]; then
    ...
    # Check if skills directory is symlinked (entire directory)
...
# Promote Codex skill to dotfiles
codex-skill-promote() {
    local skill_name="$1"
    if [ -z "$skill_name" ]; then
    ...
    # Check if entire skills directory is symlinked
    if [ -L "$HOME/.codex/skills" ]; then
